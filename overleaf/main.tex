...
\paragraph{Hierarchical Classification.} Concept predictions are computed through cosine similarity to prototypes with learned temperature and bias terms:
\begin{align}
...
\paragraph{Loss Functions.} Stage 1 employs focal loss \parencite{lin2017focal} for classification to address class imbalance, with label smoothing of 0.2 for regularization:
\begin{equation}
...
\paragraph{Prediction Heads.} Two prediction heads operate on the fused representation. A cell classification head predicts the geocell membership through softmax over 1,048 classes. An offset regression head predicts the 3D Cartesian offset from the predicted cell center to the actual location.

\paragraph{Ablation Modes.} To understand component contributions, we implement three inference modes:
\begin{itemize}
...
Figure~\ref{fig:stage1_results} visualizes per-concept performance, revealing that accuracy varies substantially across concepts---common categories like ``Urban Street'' achieve higher accuracy than rare categories due to training data distribution.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\textwidth]{analysis/stage1_accuracies.png}
    \caption{Stage 1 concept classification accuracy breakdown. Per-concept accuracy varies with training data frequency, with common concepts achieving higher recognition rates.}
...
& Concept-only & 139.0 & 745.9 & 0.451 & 0.195 & 0.566 & 0.824 \\
& Image-only & 222.0 & 1070.5 & 0.374 & 0.175 & 0.482 & 0.753 \\
\midrule
\multirow{3}{*}{Finetuned} 
& Both & \textbf{126.0} & \textbf{684.6} & \textbf{0.449} & \textbf{0.232} & \textbf{0.578} & 0.829 \\
& Concept-only & 137.0 & 688.6 & 0.443 & 0.227 & 0.564 & 0.822 \\
& Image-only & 154.0 & 790.5 & 0.430 & 0.202 & 0.546 & 0.806 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Stage 2 geolocation performance on the out-of-distribution GeoGuessr dataset. Performance degrades compared to in-distribution data, but the finetuned variant maintains superior performance.}
\label{tab:stage2_hf}
\begin{tabular}{llcccccc}
\toprule
\textbf{Variant} & \textbf{Mode} & \textbf{Median (km)} & \textbf{Mean (km)} & \textbf{Cell Acc} & \textbf{City} & \textbf{Region} & \textbf{Country} \\
\midrule
\multirow{3}{*}{Vanilla} 
& Both & \textbf{352.4} & \textbf{1538.8} & \textbf{0.266} & \textbf{0.035} & \textbf{0.362} & \textbf{0.696} \\
& Concept-only & 372.2 & 1660.1 & 0.262 & 0.037 & 0.348 & 0.671 \\
& Image-only & 447.3 & 1851.6 & 0.222 & 0.030 & 0.307 & 0.630 \\
\midrule
\multirow{3}{*}{Finetuned} 
& Both & 389.7 & 1626.2 & 0.235 & 0.031 & 0.320 & 0.674 \\
& Concept-only & 404.8 & 1716.3 & 0.221 & 0.036 & 0.315 & 0.649 \\
& Image-only & 375.1 & 1663.5 & 0.243 & 0.032 & 0.339 & 0.671 \\
\bottomrule
\end{tabular}
\end{table}

Out-of-distribution performance shows interesting patterns. Median error increases substantially (352-447 km vs 126-222 km), reflecting distribution shift between training and GeoGuessr imagery. Surprisingly, the vanilla variant outperforms finetuned on this dataset, suggesting that Stage 0 pretraining may induce some overfitting to the training distribution.

The concept-only mode remains competitive with full cross-attention, further validating that learned concepts capture transferable geographic semantics. Country-level accuracy remains above 65\% despite the distribution shift, indicating robust coarse localization.
...
Figure~\ref{fig:ablation} presents a comparative analysis of the three ablation modes for the finetuned variant.

\begin{figure}[t]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{analysis/stage2_finetuned_radar.png}
        \caption{Radar plot comparing performance metrics across ablation modes}
        \label{fig:ablation_radar}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{analysis/stage2_finetuned_lollipop.png}
        \caption{Lollipop chart showing metric-specific comparisons}
        \label{fig:ablation_lollipop}
    \end{subfigure}
...
The radar plot reveals that the ``both'' configuration dominates across most metrics, with concept-only closely following. Image-only lags behind, particularly on city-level accuracy, suggesting that semantic concepts provide more robust localization cues than raw visual features alone.
