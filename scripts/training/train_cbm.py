...
def visualize_predictions(model, dataset, device, epoch, output_dir, phase):
    """
    ...
    for i, idx in enumerate(indices):
        ...
        with torch.no_grad():
            ...
            c_logits, cell_logits, pred_offsets = model(pooled_dev)
        ...
        bar_ax.barh(y_pos, top5_prob.cpu().numpy(), color='steelblue')
    ...
    plt.savefig(viz_dir / f"epoch_{epoch}_phase{phase}.png")
    plt.close()
...
def train_epoch(model, loader, optimizer, device, phase, concept_criterion, cell_criterion, offset_criterion):
    ...
    for pooled, _, c_labels, _, cell_labels, offsets in pbar:
        ...
        # Forward
        c_logits, cell_logits, pred_offsets = model(pooled)
        ...
        # Phase 3: Joint Loss
            ...
            # Combined
            loss = c_loss + 0.5 * geo_loss # Weight geo loss less initially? Or 1.0?
    ...
    return total_loss / total_samples, correct_c / total_samples, correct_cell / total_samples
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, train_acc_c, train_acc_cell = train_epoch(
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0:
            ...
            visualize_predictions(model, val_ds, device, epoch, output_dir, args.phase)
