...
if [ "${VANILLA_MODE}" -eq 1 ]; then
    ...
    if [ "${MODE}" == "" ] || [ "${MODE}" == "all" ]; then
        ...
        sbatch  jobs/stage2/vanilla_stage1/train_stage2_ablation_concept_only_vanilla_stage1.job

        echo "[2/3] Submitting image_only experiment (vanilla Stage 1)..."
        sbatch  jobs/stage2/vanilla_stage1/train_stage2_ablation_image_only_vanilla_stage1.job

        echo "[3/3] Submitting both experiment (vanilla Stage 1)..."
        sbatch  jobs/stage2/vanilla_stage1/train_stage2_ablation_both_vanilla_stage1.job
...
if [ "${MODE}" == "" ] || [ "${MODE}" == "all" ]; then
    ...
    sbatch jobs/stage2/ablations/train_stage2_ablation_concept_only.job

    echo "[2/3] Submitting image_only experiment..."
    sbatch jobs/stage2/ablations/train_stage2_ablation_image_only.job

    echo "[3/3] Submitting both experiment..."
    sbatch jobs/stage2/ablations/train_stage2_ablation_both.job
